#!/usr/bin/env python3
"""source-doc-context — SessionStart documentation slice (Claude Code only).

stdout is injected as each session's opening context. Materializes the next ring of
progressive disclosure — the context AGENTS.md only points at — so a session boots
already holding it. AGENTS.md/CLAUDE.md are free (always-on) and deliberately NOT
re-pushed here.

Tune what each session boots with by editing the slice list at the bottom of main():
the `section(...)` / `module(...)` calls each target a canonical sliceable header
(see docs/README.md "Canonical headers (sliceable)"). The /refine-context skill drives this.
"""
...
def main() -> int:
    ...
    module("frontend")
    module("cdk")
    # docs renders like a module (its `## Overview`), then appends the documentation
    # index — docs/README.md's own `## Module Map`.
    module("docs")
