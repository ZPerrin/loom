...
As a human, my pattern recognition is typically leaning into getting my bearings with a 20,000ft view first, and only then progressively diving into the details as needed.  You can see the same patterns emerge with agentic search and tool calls.  It works for both parties really well, so we want to reinforce that pattern while keeping it useful and optimized for both.
...
For a human, that means never scavenger-hunting for a fact or maintaining an index that's stale the moment I commit new code — the right doc should sit where I'd already think to look, and read like it belongs there.
...
`docs/config/loom.toml`; the runtime is bash + awk with no external dependencies.
