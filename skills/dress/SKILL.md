...
- For each `hook` the confirmed config names that points to a script (not an inline command), scaffold an executable stub under `scripts_dir` from [templates/hook-stub.sh](templates/hook-stub.sh) if absent, creating `scripts_dir` as needed. Hook scripts are not managed docs — the linter checks markdown — so they carry no frontmatter.
...
Report the confirmed plan (modules, doc homes, config, pivots), what was seeded vs. mapped vs. flagged as a gap, the `loom.toml` written, any hook stubs scaffolded under `scripts_dir`, the `dress.md` if a divergence warranted one, gaps flagged for `/weft`, and the clean lint run.
