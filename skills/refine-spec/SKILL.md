...
- **Never a silent rewrite.** This skill writes only change-log lines and the stamp to a repo spec, the tier the grammar gives a reconciliation pass. Recommend a direction for each finding, and write only what the operator ruled.
