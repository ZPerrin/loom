...
3. Move the roadmap only on milestone events: update `## Now`/`## Next`, check off `## Milestones` in `docs/config/roadmap.md`. No per-session entry — git is the activity log.
...
6. Run the bundled linter `bash "${CLAUDE_PLUGIN_ROOT}/scripts/doc-linter"` and fix what it flags (links + the frontmatter/stamp two-tier); `git add` the doc changes.
...
8. **Commit the sync** on the work branch: `git commit -m "docs(weft): <what was distilled>"` (project trailer per the repo's commit convention).
