#!/usr/bin/env bash
# warp-open.sh — the graduated warp preamble: branch + worktree + env-source, one shot.
#
...
#
#   usage: warp-open.sh <slug> [base-branch]
#
...
echo "warp-open: branch=${branch} base=${base} worktree=${wt_path}"
...
for env_file in "${wt_path}/.env" "${wt_path}/.env.local"; do
  if [[ -f "${env_file}" ]]; then
    echo "warp-open: sourcing ${env_file}"
    # shellcheck disable=SC1090
    set -a; . "${env_file}"; set +a
  fi
done

echo "warp-open: ready — cd ${wt_path} (on ${branch})"
